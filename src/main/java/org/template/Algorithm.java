...
import static org.template.Conversions.drawInfo;
...
public class Algorithm extends P2LJavaAlgorithm<PreparedData, NullModel, Query, PredictedResult> {
  ...
  public Algorithm(AlgorithmParams ap) {
    ...
    this.logAlgorithmParameters();

  }

  /**
   * Log the parameters onto the console when in INFO mode
   */
  private void logAlgorithmParameters() {
    ...
    parameters.add(new Tuple2<>("MaxEventsPerEventType", maxEventsPerEventType));
    parameters.add(new Tuple2<>("==========", "============================"));
    ...
    parameters.add(new Tuple2<>("==========", "============================"));
    parameters.add(new Tuple2<>("Rankings", rankingParams.stream().map(rankingParams -> rankingParams.getName() + " ").collect(Collectors.joining(","))));
    drawInfo("Init URAlgorithm", parameters, logger);
  }
  ...
  private static class BoostableCorrelators {
    ...
    protected final List<String> itemIDs; // itemIDs
    public final Float boost;

    protected BoostableCorrelators(String actionName, List<String> itemIDs,
                                   Float boost) {
  ...
  private NullModel calcPop(SparkContext sc, PreparedData data) {
    ...
    for (String action : queryEventNames) {
      ...
      for (Event e : recentEvents) {
        if (e.event().equals(action) && items.size() < maxQueryEvents && !e.targetEntityId().isEmpty()) {
          items.add(e.targetEntityId().get()); // converting Option<String> to JAVA string, since Event is a native pio scala class
        }
      }
  ...
  private List<JsonElement> buildQueryShould(Query query, List<BoostableCorrelators> boostable) {
    ...
    for (BoostableCorrelators bc : recentUserHistory) {
      ...
      innerObj.add(bc.actionName, jsonelement);
